...
function buildPrompt(
  ...
  if (implementationAgents.includes(step.agentId)) {
    ...
    prompt += `\n\n---${autoApprove}\n### TOOL ACCESS\n- **list_files**: Browse directories\n- **read_file**: Read file (use line_start/line_end for large files)\n- **edit_file**: Surgical edit (old_string → new_string)\n- **create_file**: New files only\n- **run_command**: \`npx tsc --noEmit\` to verify\n\n### STRATEGY (CRITICAL — follow this exactly)\n1. Read ONLY the file you need to change (1-2 files max)\n2. Read ONLY the section you need to edit (use line_start/line_end)\n3. Make edit_file call with minimal change\n4. If task requires changes in >2 files: pick the MOST IMPORTANT one, edit it, and describe remaining changes in your summary\n5. Do NOT try to implement everything — focus on the core change\n\n### BUDGET: Max 5 read calls, then you MUST edit or finish.\n\n### DEFINITION OF DONE\n1. At least one edit_file or create_file succeeded\n2. Provide summary of what you changed and what remains to be done`;
